package net.myerichsen.gedcomdb;
...
public class Fonkod {
	...
	private String finalizeProcessing(String input) {
		...
		// (C^s)...s/V
		out = out.replaceFirst("([aeiouyæøå]+[^aeiouyæøå]*[^aieosuyæøå])[^aeiosuyæøå]*([aeiosuyæøå]).*", "$1$2");
	...
	private String normaliseForeignSpellings(String input) {
		...
		out = out.replaceAll("ö", "ø");
		out = out.replaceAll("ž", "sj");
	...
	private String processFirstConsonantGroup(String input) {
		...
		// c -> k
		out = out.replaceFirst("\\b([^aceiouyæøå]*)c", "$1k");
